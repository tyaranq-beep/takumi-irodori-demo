...
import { FadeUp, FadeLeft } from '../components/Motion';
            ...
            <FadeUp>
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                    <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}>
                        <Link to={href} className="bg-slate-950 hover:bg-slate-800 text-white px-8 py-4 rounded-full text-base font-bold transition-all shadow-lg block">
                            {text}
                        </Link>
                    </motion.div>
                    <motion.a
                        href="https://line.me"
                        className="bg-[#06C755] hover:bg-[#05b04a] text-white px-8 py-4 rounded-full text-base font-bold flex items-center justify-center gap-2 transition-all shadow-lg"
...
export default function Home() {
    ...
    return (
        <div>
            {/* ヒーロー - stagger entrance animation */}
                ...
                <div className="relative z-10 max-w-7xl mx-auto px-6 w-full py-20">
                    ...
                    >
                        {[
                            ...
                            <motion.h1
                                key="h1"
                                className="text-4xl md:text-6xl lg:text-7xl font-serif text-white font-bold leading-[1.15] mb-8 drop-shadow-xl"
                                variants={{ hidden: { opacity: 0, y: 30 }, show: { opacity: 1, y: 0, transition: { duration: 0.8, ease: [0.22, 1, 0.36, 1] as [number, number, number, number] } } }}
                            >
                                この街で、いちばん<br />
                                <span className="text-indigo-400">「ありがとう」と言われる</span><br />
                                工務店でいたい。
                            </motion.h1>,
                            <motion.p
                                key="lead"
                            ...
                            >
                                <motion.div whileHover={{ scale: 1.04, y: -2 }} whileTap={{ scale: 0.97 }}>
                                    <Link to="/contact" className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-4 rounded-full text-lg font-bold text-center transition-all shadow-xl block">
                                        まず無料でお見積り →
                                    </Link>
                                </motion.div>
                                <motion.a
                            ...
                            >
                                {['しつこい営業一切なし', '最短即日お伺い', '見積もり・診断 完全無料'].map((t, i) => (
                                    <div key={i} className="flex items-center gap-2 bg-slate-900/60 backdrop-blur-sm px-4 py-2 rounded-full border border-white/10 text-sm text-white/90">
                                        <CircleCheck className="w-4 h-4 text-indigo-400" />{t}
                                    </div>
                                ))}
                            </motion.div>
                        ]}
            ...
            <section className="py-12 bg-slate-900 border-y border-white/5">
                <div className="max-w-5xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
                    {[
                        ...
                        { Icon: Star, title: 'Googleレビュー', sub: '★4.8（42件）' },
                        ...
                        <FadeUp key={i} delay={i * 0.08}>
                            ...
                            >
                                <Icon className="w-10 h-10 text-indigo-400 mb-1" />
            ...
            <section className="py-24 bg-slate-950">
                <div className="max-w-7xl mx-auto px-6">
                    <FadeUp className="text-center mb-16">
                        <span className="text-indigo-400 font-bold tracking-widest text-xs uppercase block mb-4">Reasons</span>
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {reasons.map((r, i) => (
                            <FadeUp key={i} delay={i * 0.1}>
                                <motion.div
                                    className="flex flex-col items-center text-center bg-slate-900 p-8 rounded-3xl border border-white/5 h-full"
                                    whileHover={{
                                        borderColor: 'rgba(99,102,241,0.4)',
                                        y: -6,
                                        boxShadow: '0 20px 40px rgba(0,0,0,0.5), 0 4px 12px rgba(99,102,241,0.15)',
                                    }}
                                    transition={{ type: 'spring', stiffness: 280, damping: 22 }}
                                >
                                    <motion.div
                                        className="w-16 h-16 rounded-full bg-indigo-500/10 flex items-center justify-center mb-6"
                                        whileHover={{ scale: 1.15, rotate: 5 }}
                                        transition={{ type: 'spring', stiffness: 400, damping: 15 }}
                                    >
                                        <r.icon className="w-7 h-7 text-indigo-400" />
                                    </motion.div>
            ...
            <section className="py-24 bg-slate-900">
                <div className="max-w-7xl mx-auto px-6">
                    <FadeUp className="flex flex-col md:flex-row items-start md:items-end justify-between gap-4 mb-14">
                        <div>
                            <span className="text-indigo-400 font-bold tracking-widest text-xs uppercase block mb-4">Works</span>
                            <h2 className="text-3xl md:text-4xl font-serif text-white font-bold">施工事例（ビフォー・アフター）</h2>
                            <div className="w-16 h-1 bg-indigo-600 mt-5 rounded-full" />
                        </div>
                        <Link to="/works" className="flex items-center gap-2 text-indigo-400 font-bold hover:gap-3 transition-all text-sm">
                            すべての事例を見る <ArrowRight className="w-4 h-4" />
                        </Link>
                    </FadeUp>
                    <FadeUp delay={0.1} className="max-w-4xl mx-auto">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4">
                            ...
                            <div className="flex-1 p-5 bg-indigo-500/10 rounded-2xl border border-indigo-500/20">
                                <p className="text-xs font-bold text-indigo-400 uppercase tracking-widest mb-2">✅ 解決のポイント</p>
            ...
            <section className="py-24 bg-slate-900">
                <div className="max-w-7xl mx-auto px-6">
                    <FadeUp className="text-center mb-16">
                        <span className="text-indigo-400 font-bold tracking-widest text-xs uppercase block mb-4">Testimonials</span>
                    ...
                    <div className="grid md:grid-cols-3 gap-8">
                        {reviews.map((r, i) => (
                            <FadeUp key={i} delay={i * 0.12}>
                                <motion.div
                                    className="bg-slate-950 rounded-3xl p-8 border border-white/5 relative group h-full"
                                    whileHover={{
                                    ...
                                    transition={{ type: 'spring', stiffness: 280, damping: 22 }}
                                >
                                    <Quote className="absolute top-6 right-6 w-10 h-10 text-white/5 group-hover:text-indigo-400/15 transition-colors" />
                                    <div className="flex gap-1 mb-5">
                                        {[...Array(5)].map((_, j) => <Star key={j} className="w-4 h-4 fill-indigo-400 text-indigo-400" />)}
                                    </div>
                                    <p className="text-slate-300 text-sm leading-relaxed mb-6">「{r.text}」</p>
                                    <div className="border-t border-white/5 pt-5">
                                        <p className="font-bold text-white text-sm">{r.name}</p>
                                        <p className="text-indigo-400 text-xs mt-1">{r.tag}</p>
                                    </div>
                                </motion.div>
                            </FadeUp>
                        ))}
